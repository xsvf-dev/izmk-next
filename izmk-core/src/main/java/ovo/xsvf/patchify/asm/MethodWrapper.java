...
public final class MethodWrapper {
    ...
    private static MethodHandle findMethod(Class<?> clazz, String methodName, String methodDesc) throws Exception {
        Pair<String, String> pair = Pair.of(Type.getInternalName(clazz) + "/" + methodName, methodDesc);
        ...
        methodName = ASMUtil.splitDesc(pair.first()).second();
    ...
    private Object invokeMethod(Object instance) {
        try {
            if (instance == null) {
                ...
                methodParams.add(0, instance);
            }
            return lookup0.invokeWithArguments(methodParams);
        } catch (Throwable e) {
