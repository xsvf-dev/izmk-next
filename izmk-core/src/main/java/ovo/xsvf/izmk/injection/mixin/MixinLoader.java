...
public class MixinLoader implements IMixinLoader {
    ...
    private static Inject getInject(Class<?> mixinClass, Method method) {
        ...
        if (!(method.getReturnType() == void.class))
            throw new IllegalArgumentException("Mixin method " + method.getName() + " in class " + mixinClass.getName() + " should return void");
        Parameter[] params = method.getParameters();
        if (params[params.length - 1].getType() != CallbackInfo.class)
            throw new IllegalArgumentException("Mixin method " + method.getName() + " in class " + mixinClass.getName() + " does not have a CallbackInfo parameter");
        if (inject.at().value() == At.Type.HEAD) {
            for (Parameter param : method.getParameters()) {
                if (param.isAnnotationPresent(Local.class))
                    throw new IllegalArgumentException("Mixin method " + method.getName() + " in class " + mixinClass.getName() + " has a @Local parameter with HEAD inject point");
            }
        }
    ...
    private static void injectHead(MethodNode method, Method inject) {
        ...
        if (!Modifier.isStatic(method.access)) insnList.add(new VarInsnNode(ALOAD, index++)); // 0
        ...
        insnList.add(new FieldInsnNode(GETFIELD, Type.getInternalName(CallbackInfo.class), "cancelled", "Z"));
        ...
        if (returnType == Type.VOID_TYPE) {
            ...
            insnList.add(new FieldInsnNode(GETFIELD, Type.getInternalName(CallbackInfo.class), "result", Type.getDescriptor(Object.class)));
            // STACK: [result]
    ...
    private static void injectTail(MethodNode method, Method inject) {
        ...
        int retOpCode = returnType.getOpcode(IRETURN);
            ...
            for (Parameter param : inject.getParameters()) {
                if (param.isAnnotationPresent(Local.class)) {
                    ...
                    if (local.value() >= method.maxLocals)
                        throw new IllegalArgumentException("Mixin method " + inject.getName() + " in class " + inject.getDeclaringClass().getName() + " has a @Local parameter with an invalid value");
    ...
    private static void injectMethod(MethodNode method, Method inject, Pair<String, String> invoke, boolean isBefore) {
        ...
        for (AbstractInsnNode insnNode : method.instructions) {
            ...
            if (insnNode instanceof VarInsnNode var && var.getOpcode() >= ISTORE && var.getOpcode() <= ASTORE) {
        ...
        for (AbstractInsnNode insnNode : toInject) {
            ...
            for (Parameter param : inject.getParameters()) {
                if (param.isAnnotationPresent(Local.class)) {
                    Local local = param.getAnnotation(Local.class);
                    if (local.value() >= method.maxLocals)
                        throw new IllegalArgumentException("Mixin method " + inject.getName() + " in class " + inject.getDeclaringClass().getName() + " has a @Local parameter with an invalid index");
                    if (!locals.containsKey(local.value()) || !locals.get(local.value()))
                        throw new IllegalArgumentException("Mixin method " + inject.getName() + " in class " + inject.getDeclaringClass().getName() + " has a @Local parameter with an index that might be not initialized in the target method");
                    Type type = Type.getType(param.getType());
                    insnList.add(new VarInsnNode(type.getOpcode(ILOAD), local.value()));
                }
            }
            ...
            insnList.add(new FieldInsnNode(GETFIELD, Type.getInternalName(CallbackInfo.class), "cancelled", Type.getDescriptor(boolean.class)));
    ...
    private static void overwrite(MethodNode method, Method inject) {
        ...
        if (Type.getReturnType(method.desc) == Type.VOID_TYPE) {
            ...
            insnList.add(new InsnNode(Type.getReturnType(method.desc).getOpcode(IRETURN)));
        }
        method.instructions.insert(insnList);
    }
    ...
    private static void wrapInvoke(MethodNode method, Method inject) {
        ...
        for (AbstractInsnNode insnNode : toWrap) {
            ...
            if (toWrap.getFirst().getOpcode() == INVOKESTATIC) {
                staticCall = true;
            } else if (toWrap.getFirst().getOpcode() == INVOKEVIRTUAL) {
                staticCall = false;
            } else {
            ...
            insnList.add(new MethodInsnNode(INVOKESTATIC, Type.getInternalName(MethodHelper.class), "getInstance", "()" + Type.getDescriptor(MethodHelper.class), false));
            // STACK: [<mInstance>, margs..., MethodHelper]
            insnList.add(new VarInsnNode(ASTORE, helperIndex));
            // STACK: [<mInstance>, margs...]
            ...
            for (int i = argTypes.length - 1; i >= 0; i--) {
                ...
                insnList.add(new VarInsnNode(ALOAD, helperIndex));
                // STACK: [<mInstance>, margs..., (Object)arg, MethodHelper]
                insnList.add(new InsnNode(SWAP));
                // STACK: [<mInstance>, margs..., MethodHelper, (Object)arg]
                insnList.add(new MethodInsnNode(INVOKEVIRTUAL, Type.getInternalName(MethodHelper.class), "addParam", "("+ Type.getDescriptor(Object.class) + ")" + Type.getDescriptor(MethodHelper.class), false));
                // STACK: [<mInstance>, margs..., MethodHelper]
                insnList.add(new InsnNode(POP));
                // STACK: [<mInstance>, margs...]
            }
            ...
            if (staticCall) {
                ...
                insnList.add(new MethodInsnNode(INVOKESTATIC, Type.getInternalName(StaticInvocation.class), "create", "(" + Type.getDescriptor(MethodHelper.class) + Type.getDescriptor(String.class) + Type.getDescriptor(String.class) + Type.getDescriptor(String.class) + ")" + Type.getDescriptor(StaticInvocation.class), false));
                // STACK: [StaticInvocation]
            } else {
                // STACK: [mInstance, MethodHelper, owner, name, desc]
                insnList.add(new MethodInsnNode(INVOKESTATIC, Type.getInternalName(VirtualInvocation.class), "create", "(" + Type.getDescriptor(Object.class) + Type.getDescriptor(MethodHelper.class) + Type.getDescriptor(String.class) + Type.getDescriptor(String.class) + Type.getDescriptor(String.class) + ")" + Type.getDescriptor(VirtualInvocation.class), false));
    ...
    private static void modifyLocals(MethodNode method, Method inject) {
        ...
        if (at.value() != At.Type.HEAD) {
            ...
            if (at.value() == At.Type.TAIL) {
                ...
                for (AbstractInsnNode insnNode : method.instructions) {
                    ...
                    if (insnNode instanceof VarInsnNode v && v.getOpcode() >= ISTORE && v.getOpcode() <= ASTORE) {
        ...
        InsnList insnList = new InsnList();
        // STACK: []
        insnList.add(new MethodInsnNode(INVOKESTATIC, Type.getInternalName(Args.class), "create", "()" + Type.getDescriptor(Args.class), false));
        // STACK: [Args]
        for (int i = 0; i < modifyLocals.indexes().length; i++) {
            ...
            insnList.add(new MethodInsnNode(INVOKEINTERFACE, Type.getInternalName(IArgs.class), "set", "(" + Type.getDescriptor(int.class) + Type.getDescriptor(Object.class) + ")" + Type.getDescriptor(IArgs.class), true));
            // STACK: [Args]
        }
        // STACK: [Args]
        insnList.add(new InsnNode(DUP));
        ...
        for (int i = 0; i < modifyLocals.indexes().length; i++) {
            ...
            insnList.add(new MethodInsnNode(INVOKEINTERFACE, Type.getInternalName(IArgs.class), "get", "(" + Type.getDescriptor(int.class) + ")" + Type.getDescriptor(Object.class), true));
            // STACK: [Args, local]
            insnList.add(ASMUtil.checkcastFromObject(type));
            // STACK: [Args, (T)local]
            insnList.add(new VarInsnNode(type.getOpcode(ISTORE), index));
            // STACK: [Args]
        }
        // STACK: [Args]
        insnList.add(new InsnNode(POP));
    ...
    @Override
    public void loadMixin(Class<?> mixinClass) throws Exception {
        ...
        for (Method method : mixinClass.getDeclaredMethods()) {
            if (method.isAnnotationPresent(Inject.class) || method.isAnnotationPresent(Overwrite.class) ||
                ...
                if (method.isAnnotationPresent(Inject.class)) {
                    Inject inject = getInject(mixinClass, method);
                    name = inject.method();
                    ...
                    if (method.getParameterCount() != 1 || !parameterTypes[0].equals(MethodNode.class))
                        throw new IllegalArgumentException("Transform method " + method.getName() + " in class " + mixinClass.getName() + " must have one parameter of type MethodNode");
                ...
                for (var injectMethod : injectMethods) {
                    ...
                    if (injectMethod.isAnnotationPresent(Inject.class)) {
                        ...
                        switch (at.value()) {
                            case HEAD -> {
                                if (Arrays.stream(injectMethod.getParameters()).anyMatch(it -> it.isAnnotationPresent(Local.class)))
                                    throw new IllegalArgumentException("Inject method " + injectMethod.getName() + " in class " + mixinClass.getName() + " has a @Local parameter, which is not allowed in HEAD position");
                        ...
                        if (index + 1 + Type.getArgumentTypes(method.desc).length > injectMethod.getParameterCount())
                            throw new IllegalArgumentException("WrapInvoke method " + injectMethod.getName() + " in class " + mixinClass.getName() + " has less parameters than the target method");
        ...
        ClassUtil.redefineClass(targetClass, ClassUtil.rewriteClass(targetNode));
    }
}
