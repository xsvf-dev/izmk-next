...
public class CustomSkyLayer {
    ...
    private static final Logger log = LogManager.getLogger(CustomSkyLayer.class);
    ...
    public CustomSkyLayer(Properties props, String defSource) {
        ...
        this.textureId = -1;
        this.lastWorld = null;
        ConnectedParser connectedparser = new ConnectedParser();
    ...
    private static void rotateDeg(PoseStack poseStack, float angle, float x, float y, float z) {
        ...
        if (str == null) {
            ...
            if (astring.length != 3) {
                ...
                if (f2 * f2 + f * f + f1 * f1 < 1.0E-5F) {
                    log.warn("Invalid axis values: " + str);
                    return defVal;
                } else {
    ...
    public boolean isValid(String path) {
        if (this.source == null) {
            log.warn("No source texture: " + path);
            return false;
        } else {
            ...
            if (this.startFadeIn >= 0 && this.endFadeIn >= 0 && this.endFadeOut >= 0) {
                ...
                int i1 = i + j + k + l;
                if (i1 != 0 && i1 != 24000) {
                    log.warn("Invalid fadeIn/fadeOut times, sum is not 24h: " + i1);
                    return false;
                } else if (this.speed < 0.0F) {
                    log.warn("Invalid speed: " + this.speed);
                    return false;
                } else if (this.daysLoop <= 0) {
                    log.warn("Invalid daysLoop: " + this.daysLoop);
                    return false;
                } else {
                    return true;
                }
            } else {
                log.warn("Invalid times, required are: startFadeIn, endFadeIn and endFadeOut.");
                return false;
            }
        }
    }
    ...
    public void render(Level world, PoseStack matrixStackIn, int timeOfDay, float celestialAngle, float rainStrength, float thunderStrength) {
        ...
        if (!(f3 < 1.0E-4F)) {
            ...
            if (this.rotate) {
                ...
                rotateDeg(matrixStackIn, 360.0F * (f4 + celestialAngle * this.speed), this.axis[0], this.axis[1], this.axis[2]);
            }
    ...
    private void addVertex(Matrix4f matrix4f, BufferBuilder buffer, float x, float y, float z, float u, float v) {
        ...
        buffer.vertex(f, f1, f2).uv(u, v).endVertex();
    }
