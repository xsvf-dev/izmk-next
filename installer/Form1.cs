    ...
    public partial class Form1 : Form
    {
        ...
        private void InitializeInstallation()
        {
            try
            {
                ...
                // 查找Java 17
                LogMessage("正在搜索Java 17或更高版本...");
                var javaExe = FindJavaExe(extPath);
                if (javaExe == null)
                {
                    // 尝试找任何版本的Java，不限制版本号
                    LogMessage("未找到Java 17，尝试搜索任何版本的Java...");
        ...
        private string FindJavaExe(string extPath)
        {
            try
            {
                LogMessage($"在 {extPath} 中搜索Java 17或更高版本...");
                ...
                return javaExes.FirstOrDefault(exe =>
                {
                    try
                    {
                        ...
                        return version.FileMajorPart >= 17;
                    }
                    catch (Exception ex)
                    {
        ...
        private void Install()
        {
            try
            {
                ...
                // 创建服务
                string cmd = $"\"{javaHome}\" -jar \"{targetJar}\" -service";
                ...
                {
                    FileName = "sc.exe",
                    Arguments = $"create {SERVICE_NAME} binPath= \"{cmd}\" DisplayName= \"{SERVICE_DISPLAY_NAME}\" start= auto type= own",
                    UseShellExecute = true,
                    Verb = "runas", // 请求管理员权限
                    CreateNoWindow = false
                };
