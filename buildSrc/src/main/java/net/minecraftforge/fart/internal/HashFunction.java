...
enum HashFunction {
    ...
    public @NotNull String hash(@NotNull File file) throws IOException {
        return hash(Files.readAllBytes(file.toPath()));
    }

    public @NotNull String hash(@NotNull Iterable<File> files) throws IOException {
    ...
    public @NotNull String hash(@Nullable String data) {
        return hash(data == null ? new byte[0] : data.getBytes(StandardCharsets.UTF_8));
    }

    public @NotNull String hash(@NotNull InputStream stream) throws IOException {
        return hash(Util.toByteArray(stream));
    }

    public @NotNull String hash(byte[] data) {
        return pad(new BigInteger(1, get().digest(data)).toString(16));
    }

    public @NotNull String pad(@NotNull String hash) {
