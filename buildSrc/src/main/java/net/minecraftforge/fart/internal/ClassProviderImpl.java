...
class ClassProviderImpl implements ClassProvider {
     ...
     */
    private final @NotNull List<FileSystem> fileSystems;
    /**
     * Holds a map of ZIP entry name / full classname -> path to the class file.
     * Always uses {@code /} for path delimiters.
     */
    private final @NotNull Map<String, Path> sources;
    /**
     * Only holds classes explicitly added through the builder with their raw class bytes.
     */
    private final @NotNull Map<String, Optional<? extends IClassInfo>> classInfos;
    /**
     * Optionally caches all class infos returned by this implementation, if not null.
     */
    private final @Nullable Map<String, Optional<? extends IClassInfo>> classCache;

    ClassProviderImpl(@NotNull List<FileSystem> fileSystems, @NotNull Map<String, Path> sources, @NotNull Map<String, Optional<? extends IClassInfo>> classInfos, boolean cacheAll) {
    ...
    static class ClassInfo implements IClassInfo {
        ...
        private final @NotNull Access access;
        private final @Nullable String superName;
        private final @NotNull List<String> interfaces;
        private final @Nullable Map<String, FieldInfo> fields;
        private Collection<FieldInfo> fieldsView;
        private final @Nullable Map<String, MethodInfo> methods;
        private Collection<MethodInfo> methodsView;

        ClassInfo(byte @NotNull [] data) {
        ...
        @Override
        public @Nullable String getSuper() {
            return superName;
        }
        @Override
        ...
        public @NotNull Access getAccessLevel() {
            return this.access;
        }

        @Override
        public @NotNull Collection<String> getInterfaces() {
            return interfaces;
        }

        @Override
        public @NotNull Collection<? extends IFieldInfo> getFields() {
            if (fieldsView == null)
        ...
        @Override
        public @NotNull Optional<? extends IFieldInfo> getField(String name) {
            return fields == null ? Optional.empty() : Optional.ofNullable(this.fields.get(name));
        }

        @Override
        public @NotNull Collection<? extends IMethodInfo> getMethods() {
            if (methodsView == null)
        ...
        @Override
        public @NotNull Optional<? extends IMethodInfo> getMethod(String name, String desc) {
        ...
        private class FieldInfo implements IFieldInfo {
            ...
            public FieldInfo(@NotNull FieldNode node) {
                this.name = node.name;
                this.desc = node.desc;
                this.access = new Access(node.access);
            }

            public FieldInfo(@NotNull Field node) {
        ...
        private class MethodInfo implements IMethodInfo {
            ...
            MethodInfo(@NotNull MethodNode node) {
                this.name = node.name;
                this.desc = node.desc;
                this.access = new Access(node.access);
            }

            MethodInfo(@NotNull Method node) {
                this.name = node.getName();
                this.desc = Type.getMethodDescriptor(node);
                this.access = new Access(node.getModifiers());
            }

            MethodInfo(@NotNull Constructor<?> node) {
                this.name = "<init>";
    ...
    private static class Access {
        private static int @NotNull [] ACC = new int[23];
        private static String @NotNull [] NAME = new String[23];
        static {
            int idx = 0;
